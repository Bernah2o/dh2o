...
class ReporteViewSet(viewsets.ModelViewSet):
    ...
    def generar_reporte_pdf(self, request, reporte_id=None):
        ...
        logo_path = (
            "authApp/static/img/logo.png"  # Ruta a tu archivo de imagen del logo
        )
            ...
            TableStyle(
                [
                    (
                        ...
                        (0, 0),
                        "TOP",
                    ),  # Alinear el logo en la parte superior
                    (
                        "VALIGN",
                        (0, 0),
                        (-1, -1),
                        "MIDDLE",
                    ),  # Centrar verticalmente todo el contenido de la tabla
                    (
                        "ALIGN",
                        (1, 0),
                        (-1, -1),
                        "CENTER",
                    ),  # Alinear el título al centro horizontalmente
                ]
            )
        )
        ...
        imagen_antes_1_path = (
            reporte.imagen_antes_lavado_1.path
            if reporte.imagen_antes_lavado_1
            else None
        )
        imagen_antes_2_path = (
            reporte.imagen_antes_lavado_2.path
            if reporte.imagen_antes_lavado_2
            else None
        )

        # Crear las imágenes de antes del lavado con las dimensiones deseadas
        imagen_antes_1 = (
            Image(imagen_antes_1_path, width=2 * inch, height=2 * inch)
            if imagen_antes_1_path
            else None
        )
        imagen_antes_2 = (
            Image(imagen_antes_2_path, width=2 * inch, height=2 * inch)
            if imagen_antes_2_path
            else None
        )

        # Obtener las imágenes de después del lavado
        imagen_despues_1_path = (
            reporte.imagen_despues_lavado_1.path
            if reporte.imagen_despues_lavado_1
            else None
        )
        imagen_despues_2_path = (
            reporte.imagen_despues_lavado_2.path
            if reporte.imagen_despues_lavado_2
            else None
        )

        # Crear las imágenes de después del lavado con las dimensiones deseadas
        imagen_despues_1 = (
            Image(imagen_despues_1_path, width=2 * inch, height=2 * inch)
            if imagen_despues_1_path
            else None
        )
        imagen_despues_2 = (
            Image(imagen_despues_2_path, width=2 * inch, height=2 * inch)
            if imagen_despues_2_path
            else None
        )
        ...
        tabla_datos = [
            ...
            [
                "",
                Table(
                    [[imagen_antes_1, Spacer(1, 0.5 * inch), imagen_antes_2]],
                    colWidths=[2 * inch, 0.2 * inch, 2 * inch],
                ),
            ],
            ["Después del lavado:", ""],
            [
                "",
                Table(
                    [[imagen_despues_1, Spacer(1, 0.5 * inch), imagen_despues_2]],
                    colWidths=[2 * inch, 0.2 * inch],
                ),
            ],
            [
                "Próxima Limpieza:",
                Paragraph(
                    str(reporte.proxima_limpieza) if reporte.proxima_limpieza else "",
        ...
        estilo_justificado = ParagraphStyle(
            ...
            if hasattr(TA_JUSTIFY, "name")
            else 4,  # Using 4 for JUSTIFY if TA_JUSTIFY is not available
        ...
        tabla.setStyle(
            [
                ...
                (
                    ...
                    (0, 0),
                    (-1, 0),
                    colors.HexColor("#1eb8d1"),
                ),  # Establecer color de fondo para la primera fila
                (
                    "BACKGROUND",
                    (0, 7),
                    (1, 7),
                    colors.HexColor("#1eb8d1"),
                ),  # Establecer color de fondo para la fila 7
                (
                    "FONTNAME",
                    (0, 0),
                    (-1, 0),
                    "Helvetica-Bold",
                ),  # Establecer tipo de letra y negrita para la primera fila
                (
                    "ALIGN",
                    (0, 0),
                    (-1, 0),
                    "CENTER",
                ),  # Alinear el contenido de la primera fila al centro
                (
                    "VALIGN",
                    (0, 0),
                    (-1, -1),
                ...
                (
                    ...
                    (0, 7),
                    (1, 7),
                ),  # Combinar la celda de la fila 7 en las columnas 0 y 1
                (
                    "SPAN",
                    (0, 4),
                    (0, 7),
                ),  # Combinar las celdas de la columna 0 desde la fila 4 hasta la fila 7
                (
                    "SPLITBEFORE",
                    (0, 7),
                    (1, 7),
                ),  # Mover la fila 8 a la siguiente página
            ]
        )
        ...
        tabla.setStyle(
            [
                (
                    ...
                    (1, 7),
                    "CENTER",
                ),  # Alinear la celda combinada de la fila 7 a la derecha
            ]
        )
        ...
        orden_de_trabajo = (
            reporte.orden_de_trabajo if reporte.orden_de_trabajo else None
        )
        ...
        def crear_hipervinculo_con_logo(url, texto, imagen):
            estilo_link = ParagraphStyle(
                ...
                textColor=colors.HexColor(
                    "#007bff"
                ),  # Cambiar el color del hipervínculo (azul)
        ...
        tabla_redes_sociales = Table(
            [
                [
                    Paragraph(
                        crear_hipervinculo_con_logo(
                            "https://www.instagram.com/dh2ocol/",
                            "Instagram",
                            logo_instagram,
                        )
                    ),
                    Paragraph(
                        crear_hipervinculo_con_logo(
                            "https://wa.me/3157484662", "WhatsApp", logo_whatsapp
                        )
                    ),
                    Paragraph(
                        crear_hipervinculo_con_logo(
                            "https://www.facebook.com/dh2ocol/",
                            "Facebook",
                            logo_facebook,
                        )
                    ),
        ...
        tabla_redes_sociales.setStyle(
            [
                ...
                (
                    ...
                    (-1, -1),
                    6,
                ),  # Agregar espacio inferior de 6 puntos en todas las celdas
            ]
        )
