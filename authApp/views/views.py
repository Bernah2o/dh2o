...
def enviar_mensaje(request):
    if request.method == "POST":
        ...
        minuto = (
            now.minute + 1
        )  # Sumamos 1 minuto para que el mensaje se envíe en el próximo minuto

        # Enviar el mensaje utilizando pywhatkit
        try:
            pwk.sendwhatmsg(telefono, mensaje, hora, minuto)
            return render(
                request, "exito.html", {"telefono": telefono, "mensaje": mensaje}
            )
        except Exception as e:
            return render(
                request,
                "error.html",
                {"mensaje": f"Error al enviar el mensaje: {str(e)}"},
            )
